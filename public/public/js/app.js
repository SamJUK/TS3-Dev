...
var alerts = {
    ...
    createAlert: function createAlert(type, text, timeout) {
        // Is the type and alias?
        if (this.isAlias(type)) type = this.getAliasValue(type);

        // Is Valid Alert
        if (this.validAlertTypes.indexOf(type) === -1) type = 'default';

        return element;
    },

    isAlias: function isAlias(type) {
        if (typeof type === 'undefined') return console.error('Type not specified! 😱');

        return this.typeAliases.hasOwnProperty(type);
    },

    getAliasValue: function getAliasValue(type) {
        if (typeof type === 'undefined') return console.error('Type not specified! 😱');
